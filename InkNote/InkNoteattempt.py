...
def draw_Pixels():
    ...
    while noting == True:
        while tsc.touched:
            point = tsc.touch
            if point["pressure"] < 30: # ignore touches with no 'pressure' as false
                continue
            #yPos = int(point["y"]*yscale)
            #xPos = int(point["x"]*xscale)
            yPos = int((point["y"]*yscale)-100)
            xPos = int(point["x"]*xscale)
            if drawColor == colorBlack:
                drawImg.circle((xPos,yPos), thickness, drawColor, drawColor, 1)
                #drawImg.putpixel((yPos,xPos),drawColor) # Edit image based on coordinates of points touched
            if drawColor == colorRed:
                drawImgRed.circle((xPos,yPos), thickness, drawColor, drawColor, 1)
...
def main():
    ...
    while drawing == True:
        if tsc.touched:
            while tsc.touched:
                point = tsc.touch
                if point["pressure"] < 80:  # ignore touches with no 'pressure' as false
                    continue
